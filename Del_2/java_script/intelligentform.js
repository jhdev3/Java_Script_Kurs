...
const patterns = {
  ...
  userGAddress: new RegExp(/^[a-ö ]+\s\w{1,4}$/i),
...
function validateDonkeyCheck() {
  ...
  if (arrayAway == null) return false;
  if (arrayHome == null) {
    // createPara("Enkel biljett");
    return true;
  }
  for (let i = 1; i < arrayAway.length; i++) {
    if (Number(arrayHome[i]) < Number(arrayAway[i])) return false;
  }
  //  console.log(arrayHome + " + " + arrayAway);
...
function homeOut() {
  if (homeInput.value === "") return "Enkel biljett";
  else return homeInput.id + ":=" + homeInput.value;
}
...
function submitTravel(event) {
  if (
    validateDonkeyCheck() &&
    patterns[travelCity.name].test(travelCity.value)
  ) {
    reseBooking.style.display = "none";
    createUserData();
  } else {
    console.log(
      "Nice Job trying to submit empty form or doing some donkey shit"
    );
    alert("Glömt något? ");
  }
  event.preventDefault();
